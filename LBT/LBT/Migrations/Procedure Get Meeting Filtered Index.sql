IF OBJECT_ID ( 'dbo.GetMeetingFilteredIndex', 'P' ) IS NOT NULL 
    DROP PROCEDURE dbo.GetMeetingFilteredIndex;
GO
CREATE PROCEDURE dbo.GetMeetingFilteredIndex @MeetingType int, @UserId int
AS
BEGIN
...
WHERE
	...
	AND @UserId IN (SELECT * FROM GetDownlineUsers(m.MainLeaderId) UNION SELECT * FROM GetDownlineUsers(m.SecondaryLeaderId));
